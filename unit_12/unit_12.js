...
function f11() {
    let out = '';
    let newArr = [];
    for(let j = 0; j < a11.length; j++){
        for(i = a11[j].length-1; i >= 0; i--) {
            out+=a11[j][i] + ' ';
          }
    }
...
function f15() {
    ...
    document.querySelector('.out-15').innerHTML = Math.max(...out);
