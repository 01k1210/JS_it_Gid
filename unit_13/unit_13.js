...
let index = document.querySelector('.i-62').value;
a6[key] = index;
...
function f7() {
   let out = document.querySelector('.out-7');
    ...
    key in a7 ? out.innerHTML = 1 : out.innerHTML = 0;
}
...
function f9() {
    let str = '';
    let v = +document.querySelector('.i-9').value;
    for( let key in a9 ){
        if ( a9[key] === v ){
            str += key;
        }
...
function f12() {
    let key = +document.querySelector('.i-12').value;
    for( let i in a12 ){
        if( a12[i] == key ){
            delete a12[i];
        }
...
function f19() {
    ...
    for(let i in a19){
        for(let a = 0; a < a19[i].length; a++){
            if(inp == a19[i][a].toLowerCase() || inp == a19[i][a]){
                res += i
            }
        }
    }
